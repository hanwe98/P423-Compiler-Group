     ...
     (ProgramDefs
      ...
      (for/list ([def defs])
        (match def
          [(Def f params rt '() exp)
           (define env^
             (append (for/list ([var-type params])
                       (match var-type
                         [`[,x : ,type]
                          (cons x (gensym x))]))
                     env))
           (define params^
             (for/list ([var-type params])
               (match var-type
                 [`[,x : ,type]
                  `[,(dict-ref env^ x) : ,type]])))
           (Def (dict-ref env f)
...
(define (uniquify-exp env)
  (lambda (e)
    (match e
      ...
      [(Prim 'vector-ref es)
       (Prim 'vector-ref (map (uniquify-exp env) es))]
      ...
      [(Apply f params)
       (Apply ((uniquify-exp env) f)
              (map (uniquify-exp env) params))]
      [(FunRef f)
...
type ::= Void
exp ::= (SetBang var exp) | (Begin (exp…) exp)
        | (WhileLoop exp exp) | (Void)
...
type ::= (type… -> type)
exp ::= (Apply exp exp…) | (FunRef var)
def ::= (Def var ([var:type]…) type ’() exp)

LfunRef ::= (ProgramDefs ’() (def …))
|#

; reveal-functions : LfunRef -> LfunRef
...
(define (rf-exp functions)
  (λ (e)
    (match e
      ...
      [(Prim op es)
       (Prim op (map (rf-exp functions) es))]
      [(Let x rhs body)
       (Let x ((rf-exp functions) rhs)
            ((rf-exp functions) body))]
    
      [(If cnd then else)
       (If ((rf-exp functions) cnd)
           ((rf-exp functions) then)
           ((rf-exp functions) else))]
    
      [(SetBang x e^)
       (SetBang x ((rf-exp functions) e^))]
      [(Begin es end)
       (Begin (map (rf-exp functions) es)
              ((rf-exp functions) end))]
      [(WhileLoop cnd body)
       (WhileLoop ((rf-exp functions) cnd)
                  ((rf-exp functions) body))]

      [(HasType e^ t)
       (HasType ((rf-exp functions) e^) t)]
    
      [(Apply f es)
       (Apply ((rf-exp functions) f) (map (rf-exp functions) es))])))
...
#;(transform-body (Prim '- (list (Var 'a) (Prim '+ (list (Var 'f) (Var 'x)))))
                (cons 'tup '(Vector Integer Integer Integer))
                `([a : Integer]
                  [b : Integer]
                  [c : Integer]
                  [d : Integer]
...
; expose-allocation : LfunRef -> LfunRef
...
(define (expose-alloc-exp e)
  (match e
    ...
    [(Let x rhs body)
     (Let x
          (expose-alloc-exp rhs)
          (expose-alloc-exp body))]
    [(Prim op es)
     (Prim op (map expose-alloc-exp es))]
    [(If cnd then else)
     (If (expose-alloc-exp cnd)
         (expose-alloc-exp then)
         (expose-alloc-exp else))]
    [(SetBang var exp)
     (SetBang var (expose-alloc-exp exp))]
    [(Begin es end)
     (Begin (map expose-alloc-exp es)
            (expose-alloc-exp end))]
    [(WhileLoop cnd body)
     (WhileLoop (expose-alloc-exp cnd)
                (expose-alloc-exp body))]
...
(define (rco-atom e)
  (match e
    ...
    [(Apply f params)
     ...
     (define alist (dict-set dict tmp value))
     (cons (Var tmp) alist)]
    [(FunRef f)
     (define tmp (gensym 'tmp))
     (define alist (dict-set '() tmp e))
...
(define (select-instructions p)
  (match-let ([(ProgramDefs types defs) (type-check-Cfun p)])
    (define new-defs
      (for/list ([def defs])
        (match def
       ...
       (for/list ([def defs])
         (match def
           [(Def f '() 'Integer info blocks)
                                             ...
                                             (cons (car pr)
                                                   (Deref (Deref-reg loc)
                                                          (cond
                                                            ...
                                                            [else (+ (Deref-offset loc) (* 8 (length used-callee)))])))]
                                            [else pr]))) var-mapping))
            ...
            (define info^ (dict-set (dict-set (dict-set (dict-set (dict-set (dict-remove info 'conflicts)
...
(define compiler-passes
  `(
    ...
    ("reveal functions" ,reveal-functions ,interp-Rfun-prime)
