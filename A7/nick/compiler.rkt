    ...
    [(ProgramDefs info defs)
     (define functions
       (for/list ([def defs])
         (match def
           [(Def f params rt '() e)
            (Def f params rt '() (reveal-casts-exp e))])))
     (ProgramDefs info functions)]))

(define (pred-association p)
  (match p
    ['boolean? 4]
    ['integer? 1]
    ['vector? 2]
...
(define (reveal-casts-exp e)
  ...
  (match e
    ...
    [(Lambda ts t exp)
     (Lambda ts t (recur exp))]
    [(Apply f a)
     (Apply (recur f) (map recur a))]
    [`(,prim^ ,op ,(list e^))
     #:when (set-member? prim^ type-predicates)
     (define tmp (gensym 'tmp))
     (If (Prim eq? (list (Prim 'tag-of-any (list (Var tmp))
                               (Int (pred-association prim^))))
               (Bool #t)
               (Bool #f)))]
    [(Project e^ ftype)
       ...
       [3 ; Function
        (match ftype
          [`(,ts ... -> ,rt)
           (Let temp (recur e^)
                (If
                 (If (Prim 'eq? (list (Prim 'tag-of-any (list (Var temp)))
                                      (Int (any-tag ftype))))
                     (Prim 'eq? (list (Prim 'procedure-arity (list (ValueOf (Var temp) ftype)))
                                      (Int (length ts))))
                     (Bool #f))
                 (ValueOf (Var temp) ftype)
                 (Exit)))])]
             
       [2 ; Vector
        (Let temp (recur e^)
             (If
              (If (Prim 'eq? (list (Int (sub1 (length ftype)))
                                   (vector-length e^)))
                  (Prim 'eq? (list (Prim 'tag-of-any (list (Var temp)))
                                   (Int (any-tag ftype))))
                  (Bool #f))
              (ValueOf (Var temp) ftype)
              (Exit)))]
       [else ; integer or boolean
        (Let temp (recur e^)
             (If (Prim 'eq? (list (Prim 'tag-of-any (list (Var temp)))
                 ...
                 (Exit)))])]
    [(Inject e^ ftype)
     (Prim 'make-any (list (recur e^) (Int (any-tag ftype))))]
...
(define (rco-exp e)
  (match e
    ...
    [(ValueOf e^ ftype)
     (define rco-e (rco-atom e^))
     (define first (car rco-e))
     (define second (cdr rco-e))
     (update (ValueOf first ftype) second)]; car and cdr for value and dictoinary
    [(FunRef f) e]))
...
(define (rco-atom e)
  (match e
    ...
    [(ValueOf e^ ftype)
     (define tmp (gensym 'tmp))
     (define sub (ValueOf (rco-exp e^) ftype))
     (define alist (dict-set '() tmp sub))
     (cons (Var tmp) alist)]
    [(SetBang x e)
     (define tmp (gensym 'tmp))
...
(define compiler-passes
  `(
    ...
    ("explicate control" ,explicate-control ,interp-Lany)
