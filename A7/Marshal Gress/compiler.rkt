...
(define (explicate-pred cnd thn els)
  (match cnd
    ...
    [(ValueOf exp t)
     (match t
       ['Boolean
        (explicate-pred exp thn els)]
       [else (error "explicate-pred : (ValueOf exp t) expected Boolean for type, got" t)])]
...
(define si-tail
  (λ (func-name tail)
    (match tail
      ...
      [(Exit)
       (list (Instr 'movq (list (Imm -1) (Reg 'rdi)))
             (Callq 'exit 1))]
    ...
    (match exp
      ...
      [(Prim 'any-vector-set! `(,tup ,n ,rhs))
       ...
       (list (Instr 'movq (list (Imm -8) (Reg 'r11)))
             ...
             (Instr 'addq (list (Reg 'rax) (Reg 'r11)))
             (Instr 'movq (list rhs^ (Deref 'r11 0)))
             (Instr 'movq (list (Imm 0) arg)))]
      ...
      [(Prim 'make-any `(,atm1 ,(Int tag)))
       (let ([arg1 (si-atm atm1)])
         (match tag
           [(or 1 4)
            (list (Instr 'movq (list arg1 arg))
                  (Instr 'salq (list (Imm 3) arg))
                  (Instr 'orq (list (Imm tag) arg)))]
           [else ; tuples and procedures memory addresses, multiple of 16
            (list (Instr 'movq (list arg1 arg))
                  (Instr 'orq (list (Imm tag) arg)))]))]
      
      [(Prim 'tag-of-any `(,atm1))
       (let ([arg1 (si-atm atm1)])
         (list (Instr 'movq (list arg1 arg))
               (Instr 'andq (list (Imm 7) arg))))]
      
      [(ValueOf `,e `,type)
       (match (any-tag type)
         [(or 1 4)
          (list (Instr 'movq (list (si-atm e) arg))
                (Instr 'sarq (list (Imm 3) arg)))]
         [else
          (list (Instr 'movq (list (Imm -8) arg))
                (Instr 'andq (list (si-atm e) arg)))])]
...
(define compiler-passes
  `(
    ...
    ("explicate control" ,explicate-control ,interp-Cany)
    ("instruction selection" ,select-instructions ,interp-pseudo-x86-4)
    ("liveness analysis" ,uncover-lives ,interp-pseudo-x86-4)
    ("build interference graph" ,build-inter-graph ,interp-pseudo-x86-4)
