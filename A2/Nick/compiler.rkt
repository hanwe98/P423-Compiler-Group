...
(define (color-graph graph locs)
  ...
  (let-values ([(x y) (update-taken-colors graph)])
    x)
  
  (for ([u (take-hs)]
        [v (in-vertices graph)]
        #:when (not (hash-has-key? location-colors (cdr u))))
    (for ([c colors]
          #:when (not (member (car c) (cdr u))))
      #:final
...
(define (update-taken-colors graph)
  (for ([location-color location-colors])
    (for ([v (in-neighbors graph (car location-color))])
      (hash-set! taken-colors
                 v
                 (cons (cdr location-color)
                       (hash-ref taken-colors v))))))

(hash-set! location-colors (Var 't) 3)
(update-taken-colors (big-loi ex (uncover-live-loi ex (set))))
...
(define (take-hs)
  (define pq
    (make-pqueue (λ (p1 p2)
                   (> (length (cdr p1))
                      (length (cdr p2))))))
  (for ([v-and-nc taken-colors]) ; vertex and neighbor colors
