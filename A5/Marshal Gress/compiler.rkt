...
exp ::= (Int int) | (Var var) | (Let var exp exp)| (Prim op (exp…))
      ...
      | (Void) | (Prim vector-ref (exp (Int int)) 
...
; expose-allocation : Rvec -> RAlloc
(define (expose-allocation p)
  (match p
    [(Program info exp)
     (Program info (expose-alloc-exp exp))]))

; expose-alloc-exp : Rvec-exp U Ralloc-exp -> Ralloc-exp
        ...
        (let* ([len (length es)]
               ...
               [rest (Let '_ (gen-collect bytes)
                          (Let 'v (Allocate len t)
                               sets))])
          (tmps-es v rest 0))])]
    ; is there a shadowing issue w/ how new vars are named?
    ; (might need to put uniquify first)
...
; remove-complex-opera* : RAlloc -> RAlloc
...
(define (select-instructions p)
  (match-let ([(CProgram types blocks) (type-check-Cvec p)])
    (X86Program types
...
(define pre-color (append
                   (list (cons (Reg 'rax) -1)
                         (cons (Reg 'rsp) -2)
                         (cons (Global 'fromspace_end) -3)
                         (cons (Global 'free_ptr) -4)) ; TODO?
    ...
    [(X86Program info blocks)
     ...
     (define instrs-handling-cmpq-movzbq
       (foldr (λ (instr rest)
        ...
        (cons (car block)
              (Block '() instrs-handling-cmpq-movzbq))))
     (X86Program
      info
      new-blocks
...
(define compiler-passes
  `(
    ("shrink" ,shrink ,interp-Rvec-prime ,type-check-Rvec)
    ("expose allocation" ,expose-allocation ,interp-Rvec-prime)
    ("uniquify" ,uniquify ,interp-Rvec-prime)
    ("remove complex opera*" ,remove-complex-opera* ,interp-Rvec-prime)
    ("explicate control" ,explicate-control ,interp-Cvec)
    ("instruction selection" ,select-instructions ,interp-pseudo-x86-2)
    ("liveness analysis" ,uncover-lives ,interp-pseudo-x86-2)
    ("build interference graph" ,build-inter-graph ,interp-pseudo-x86-2)
