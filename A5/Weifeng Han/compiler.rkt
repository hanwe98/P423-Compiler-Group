...
(define (expose-allocation p)
  (match p
    [(Program info e) (Program info (ea-exp e))]))
...
(define (select-instructions p)
  (match-let
      ([(CProgram info blocks) (type-check-Cvec p)])
  ...
  (match p
    [(X86Program info labels-blocks)
     ...
     (define (trans block-label live-after)
       (define block (dict-ref labels-blocks block-label))
       (match block
         [(Block binfo instrs)
     ...
     (define newblocks
       (map (λ (pr)
              (begin
                ...
                (cons label (Block '() instrs-handling-cmpq-movzbq))))
            blocks))
     (X86Program info newblocks)]))
...
(define (build-conclusion info)
  (let ([setup
         (append
          (list
           (Instr 'subq (list (Imm (* 8 (dict-ref info 'num-root-spills))) (Reg 'r15))) ;; relating to rootstack
...
(define compiler-passes
  `( ("shrink" ,shrink ,interp-Rvec ,type-check-Rvec)
     ("uniquify" ,uniquify ,interp-Rvec ,type-check-Rvec)
     ("expose-allocation" ,expose-allocation ,interp-Rvec-prime ,type-check-Rvec)
     ("remove-complex" ,remove-complex-opera* ,interp-Rvec-prime ,type-check-Rvec) 
