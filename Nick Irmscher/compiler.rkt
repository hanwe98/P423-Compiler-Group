  ...
  (match p
    [(Program info body)
     (let ([r (explicate-tail body)])
       (CProgram (cdr r) (list (cons 'start (car r)))))]))
  ...
  (match p
    [(CProgram info body)
     (let* ([pr (car body)]
            [label (car pr)]
            [tail (cdr pr)])
       (X86Program info (list (cons label
...
(define (assign-homes p)
  (match p
    [(X86Program info blocks)
     (let ([assignedvars (assigned-vars 0 info '())])
       (X86Program (list (car assignedvars))
                   (list (cons (car (car blocks))
                               (Block '()
                                      (match (cdr (car blocks))
                                        [(Block a ls) (map-homes (car (cdr assignedvars)) ls)]))))))]))

(define (assigned-vars acc info dict)
  (cond
    [(empty? info) (list (cons 'stack-space (+ 8 (- acc))) dict)]
    [else (let* ([curr (car info)]
                 [acc (- acc 8)]
                 [info (cdr info)]
                 [dict (dict-set dict curr acc)])
            (assigned-vars acc info dict))]))

(define (map-homes vars instrs)
  (match (car instrs)
    [(Instr op `,oper)
     (cons
      (Instr op
             (map (lambda (a)
                    (match a
                      [(Var v)
                       (Deref 'rbp (dict-ref vars
                                             (match v
                                               [(Var v) v]
                                               [else v])))]
                      [else a]))
                  oper))
      (map-homes vars (cdr instrs)))]
    [else (cons (car instrs) '())]))
