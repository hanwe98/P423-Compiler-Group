...
(define (select-instructions p)
  (match-let
      ([(CProgram info blocks) (type-check-Cif p)])
...
(define (uncover-live p)
  (match p
    [(X86Program info blocks)
     ...
     (dict-set! label->live 'conclusion (set (Reg 'rax) (Reg 'rsp))) ;; conclusion is not yet a block, so we set it manually. 
     ;; blocks : The blocks are reverse-topologically sorted.
     ;; for every block in blocks, if there is a instruction of jmp or jmpif,
     ;; create an edge pointing from the label of the block to label of the desitnation block
     (define edge-list (create-edges blocks)) ;; create all edges
     (define graph (transpose (make-multigraph edge-list))) ;; create the graph from the edges and transpose the graph
     (define blocks^ (filter (λ (l) (not (equal? l 'conclusion))) (tsort graph))) ; [ListOf Label] without 'conclusion
     (define newblocks
       (for/list ([label blocks^])
         (define block (dict-ref blocks label)) 
         (match block
           [(Block binfo instrs)
            (define newbinfo (ul-instrs instrs))
            (dict-set! label->live label (car newbinfo))
            (cons label (Block newbinfo instrs))])))
...
(define (create-edges blocks)
  (foldr (λ (pair rest) (begin
                          (define label (car pair))
                          (define block (cdr pair))
                          (define current-edges (match block
                                                  [(Block binfo instrs)
                                                   (create-edges-helper label instrs)]))
...
(define instructions (list
                      (Callq 'read_int 0)
                      (Instr 'movq (list (Reg 'rax) (Var 'a70010)))
                      (Callq 'read_int 0)
                      (Instr 'movq (list (Reg 'rax) (Var 'b70011)))
                      (Callq 'read_int 0)
                      (Instr 'movq (list (Reg 'rax) (Var 'c70012)))
                      (Instr 'movq (list (Var 'b70011) (Var 'tmp70070)))
                      (Instr 'addq (list (Var 'c70012) (Var 'tmp70070)))
                      (Instr 'movq (list (Var 'a70010) (Reg 'rax)))
                      (Instr 'addq (list (Var 'tmp70070) (Reg 'rax)))
                      (Jmp 'conclusion)))
(define binfo (list
               (set (Reg 'rsp))
               (set (Reg 'rsp) (Reg 'rax))
               (set (Reg 'rsp) (Var 'a70010))
               (set (Reg 'rsp) (Var 'a70010) (Reg 'rax))
               (set (Reg 'rsp) (Var 'b70011) (Var 'a70010))
               (set (Reg 'rsp) (Var 'b70011) (Var 'a70010) (Reg 'rax))
               (set (Reg 'rsp) (Var 'b70011) (Var 'c70012) (Var 'a70010))
               (set (Reg 'rsp) (Var 'tmp70070) (Var 'c70012) (Var 'a70010))
               (set (Reg 'rsp) (Var 'tmp70070) (Var 'a70010))
               (set (Reg 'rsp) (Var 'tmp70070) (Reg 'rax))
               (set (Reg 'rsp) (Reg 'rax))
               (set)))
(define edges (bi instructions (cdr binfo)))
...
(define compiler-passes
  `( ("shrink" ,shrink ,interp-Rif ,type-check-Rif)
     ...
     ("remove-complex" ,remove-complex-opera* ,interp-Rif ,type-check-Rif) 
